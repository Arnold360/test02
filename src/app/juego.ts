...
export class Juego implements OnInit, OnDestroy {
 ...
 update(canvas: HTMLCanvasElement, deltaTime: number) {
   ...
   /* ajustar posicion de la pelota para que no pase demasiado el limite y
    ...
    if (this.x + this.radius > canvas.width || this.x - this.radius < 0 || (this.radius + 20 > this.x && this.y > this.realLeftPaddleY + this.radius && this.y + this.radius  > this.realLeftPaddleY + 25 )) {
      this.dx = -this.dx;
    }
